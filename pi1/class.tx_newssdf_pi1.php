...
/**
 * Plugin 'News Sdf' for the 'newssdf' extension.
 *
 * @Rebuild Yann Berton <yann.berton@adico.fr>
 * @author	Philip Almeida <philip.almeida@gmail.com>
 * @package	TYPO3
 * @subpackage	tx_newssdf
 */
class tx_newssdf_pi1 extends tslib_pibase {


    var $prefixId       = 'tx_ttnews';		// Same as class name
    var $scriptRelPath	= 'pi1/class.tx_newssdf_pi1.php';	// Path to this script relative to the extension dir.
    var $extKey		= 'newssdf';	// The extension key.
    ...
    function main($main,$conf) {
        ...
        */
        ...
        $this->jsNewssdf	= str_replace(PATH_site,'',t3lib_div::getFileAbsFileName($this->conf['file.']['jsNewssdf']));
        ...
        if ( $this->conf['render.']['ie7compat'] ) {
            $ie7compat = "<!-- EXT:newssdf: IE8 Tip Compatibility: START --> " . "\n" .
                                        '<meta http-equiv="X-UA-Compatible" content="IE=7" />' . "\n" .
                                        "<!-- EXT:newssdf: IE8 Tip Compatibility: END --> " . "\n\n";
        }

        $GLOBALS['TSFE']->additionalHeaderData['tx_newssdf_inc']
                =   "\n" . 
                $ie7compat . 
                "<!-- EXT:newssdf: Javascript and CSS include files : START --> " . "\n" .
                ...
                '<script type="text/javascript" src="' . $this->jsNewssdf . '"></script>' . "\n" .
                '<!-- EXT:newssdf: Javascript and CSS include files : END --> ' . "\n";
    ...
    function buildCalendarArray() {
        ...
        if ( $this->displayType == 4 ) {
            $queryYearMonth = 'AND ( (tx_newssdf_calendardate=0 AND datetime >= '.$firstDate.' ) '.
                    'OR ( tx_newssdf_calendardate_end=0 AND tx_newssdf_calendardate >= '.$firstDate.' ) '.
                    'OR ( tx_newssdf_calendardate_end >= '.$firstDate.' ) )';
        }
        else {
            $queryYearMonth = 'AND ( (tx_newssdf_calendardate=0 AND datetime >= '.$firstDate.' AND datetime <= '.$lastDate.' ) '.
                    'OR ( tx_newssdf_calendardate_end=0 AND tx_newssdf_calendardate >= '.$firstDate.' AND tx_newssdf_calendardate <= '.$lastDate.' ) '.
                    'OR ( tx_newssdf_calendardate <= '.$lastDate.' AND tx_newssdf_calendardate_end >= '.$firstDate.' ) )';
        }
        ...
        $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                ...
                '',
                'tx_newssdf_calendardate, tt_news.datetime ASC',
                $limit
        );
    ...
    function calendarEngine() {
        ...
        $contextScript .= "\n";
        $contextScript .= "\t" ."<!-- Newssdf: Activate tooltip --> " . "\n\n";
        ...
        $contextScript .= "\t\t" ."newssdf.tipSetup(" . $tipArgumentList . ");" . "\n";
        ...
        if( is_array($this->resultList ) ) {
            ...
            while ( list( $key, $val ) = each( $this->resultList ) ) {
                ...
                if ( ! is_array($renderedDays[ $val['monthday'] ] ) ) {
                    // Create tooltip
                    $contextScript .= "\t\t" . "newssdf.processToolTip( " . $dateTime . " )" . "\n";
                }
                ...
                if ($this->thisDay==$dateTime && $this->thisYear==$this->calendarYear && $this->thisMonth==$this->calendarMonth) {
                    ...
                    $daysLinkArray[$dateTime] = array('idMenusdf'.$dateTime,'linked_today');

                } else {

                    $daysLinkArray[$dateTime] = array('idMenusdf'.$dateTime,'linked_day');
	...
	* Set tt_news to the previous id so it sticks on navigation.
    ...
    function buildContextMenu( $records ) {
        ...
        while ( list( $key, $dateItem ) = each( $records ) ) {
            ...
            if ( $checkDay != $key ) {
                ...
                $contextMenuScript .= "\n" . "\t". "<!-- Rendering newssdf item " . $currentMenu . " --> \n\n";
                $contextMenuScript .= "\t"		. "<div class='newssdfTooltip' id='toolTipIdMenusdf" . $checkDay . "'>" . "\n";
                $contextMenuScript .= "\t\t"	. "<div class='newssdfTooltipHeader'>" . "\n";
                $contextMenuScript .= "\t\t\t"	. $displayDate . "\n";
                $contextMenuScript .= "\t\t"	.  "</div>" . "\n";
                $contextMenuScript .= "\t\t"	.  "<div class='newssdfTooltipItemContainer'>" . "\n";
    ...
    function listView() {
        ...
        $fullTable			= '';	// Clear var;
        $templateMarker                 = '###NEWSSDF_LISTVIEW###';
    ...
    function makelist() {
        ...
        $templateMarker	= '###NEWSSDF_LISTVIEW_ITEM_HEADER###';
        $template	= $this->cObj->getSubpart($this->listViewTemplate, $templateMarker);
    ...
    function GetDays($sStartDate, $sEndDate, $id) {
        if ($sStartDate > $sEndDate) {
            echo "NEWSSDF ERROR: Startdate > EndDate<br />";
    ...
    function makeListItemNormal( $type = 'list' ) {
        ...
        $templateMarker = '###NEWSSDF_LISTITEM###';
        switch( $type ) {
            case 'listinterval':
                $templateMarker = '###NEWSSDF_LISTITEM_INTERVAL###';
                break;
            case 'calendar':
                $templateMarker = '###NEWSSDF_CALENDARITEM###';
                break;
        }
        ...
        if($this->getFieldContent('tx_newssdf_calendardate')!=0) {
            $marker['###STARTENDATE###'] = $this->convertSpecialCharacters(strftime($this->conf['listView.']['strftime.']['header'],$this->getFieldContent('tx_newssdf_calendardate')));
            $marker['###DAYNB###'] = $this->convertSpecialCharacters(trim(strftime("%e",$this->getFieldContent('tx_newssdf_calendardate'))));
            if ($this->getFieldContent('tx_newssdf_calendardate') != $this->getFieldContent('tx_newssdf_calendardate_end') && $this->getFieldContent('tx_newssdf_calendardate_end') !=0) {
                $marker['###STARTENDATE###'].=" ".$this->pi_getLL("dateToDate")." ".$this->convertSpecialCharacters(strftime($this->conf['listView.']['strftime.']['header'],$this->getFieldContent('tx_newssdf_calendardate_end')));
            }
        }
        else {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/newssdf/pi1/class.tx_newssdf_pi1.php']) {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/newssdf/pi1/class.tx_newssdf_pi1.php']);
}
