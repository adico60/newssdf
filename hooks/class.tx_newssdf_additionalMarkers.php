...
//****************************************************************//
class tx_newssdf_additionalMarkers {
...
//gregory goidin - rvvn : start : load the locallang value of dateToDate
		...
		$dateToDate = $lang->sL('LLL:EXT:newssdf/pi1/locallang.xml:dateToDate');
//gregory goidin - rvvn : stop !
//****************************************************************//
		...
		if ($row['tx_newssdf_calendardate'] == 0) {
			$markerArray['###NEWS_STARTDATE###'] = '';
		}
		else {
			$markerArray['###NEWS_STARTDATE###'] = $this->local_cObj->stdWrap( $row['tx_newssdf_calendardate'], $lConf['date_stdWrap.'] );
		}
		...
		if ($row['tx_newssdf_calendardate_end'] == 0) {
			...
			$markerArray['###NEWS_ENDDATE###'] = $this->local_cObj->stdWrap($row['tx_newssdf_calendardate_end'], $lConf['date_stdWrap.']);
		}
//****************************************************************//
//gregory goidin - rvvn : start : Add marker startendate
		if($row['tx_newssdf_calendardate']!=0)
		{
			$markerArray['###NEWS_STARTENDATE###'] = $this->local_cObj->stdWrap($row['tx_newssdf_calendardate'], $lConf['date_stdWrap.']);
			if ($row['tx_newssdf_calendardate'] != $row['tx_newssdf_calendardate_end'] && $row['tx_newssdf_calendardate_end']!=0)
			{
				$markerArray['###NEWS_STARTENDATE###'].=" ".$dateToDate." ".$this->local_cObj->stdWrap($row['tx_newssdf_calendardate_end'], $lConf['date_stdWrap.']);
			}
		}
		else
		{
